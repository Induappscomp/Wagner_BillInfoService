...
namespace TradingBell.WebCat.CommonServices
{
    ...
    public class PaymentServices 
    {
        ...
        //To check Transaction insert count
        public int GetTransactionCount(string transid)
        {
            int retval = 0;
            DataSet ds = new DataSet();
            try
            {
                objSqlCommand = new SqlCommand("STP_TBWC_GetTransactionCount", objConnection.GetConnection());
                objSqlCommand.CommandType = CommandType.StoredProcedure;
                objSqlCommand.Parameters.Add(new SqlParameter("@TransID", transid));
                objDataAdapter = new SqlDataAdapter(objSqlCommand);
                objDataAdapter.Fill(ds);
                ObjErrorHandler.CreatePayLog("Outside ds");
                if (ds != null)
                {
                    ObjErrorHandler.CreatePayLog("Inside ds");
                    ObjErrorHandler.CreateLog();
