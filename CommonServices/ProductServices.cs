...
namespace TradingBell.WebCat.CommonServices
{
    ...
    public class ProductServices
    {
        ...
        public int GetProductSOH(int ProductID)
        {
            int retVal = 0;
            try
            {
                string tempstr = (string)objProductDB.GetGenericDataDB(ProductID.ToString(), "GET_PRODUCT_SOH", ProductDB.ReturnType.RTString);
                if (tempstr != null && tempstr != "")
                    retVal = objHelperServices.CI(tempstr);
